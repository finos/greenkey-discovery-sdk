...
def patch_entity_list_duplicates(unique_list, original_list, i):
  ...
  indices_to_change = [index for index in range(len(unique_list)) if unique_list[index] == current_entity]
  ...
  for index in indices_to_change:
    ...
    if len(original_entity) < len(current_entity.split(' ')) + 1:
      print("WARNING: Could not remove duplicate {0} with entity {1}".format(unique_list[i], original_entity))
      possible_conflicts.append(index)
    else:
      unique_list[index] = ' '.join(original_entity[:len(current_entity.split(' ')) + 1])

  if len(possible_conflicts) > 1:
    print("ERROR: More than one duplicate could not be resolved at entity {0} of {1}".format(i + 1, len(original_list)))
...
def create_unique_minimum_entities(entities):
  ...
  while True:
    ...
    while i < len(unique_entity_working_list):
      if unique_entity_working_list[i] in seen_entities:
        first_duplicate = seen_entities.index(unique_entity_working_list[i])
        unique_entity_working_list = patch_entity_list_duplicates(unique_entity_working_list, original_entities, i)
        seen_entities[first_duplicate] = unique_entity_working_list[first_duplicate]
...
def generate_ticker_file(filename, case_insensitive=False, pickled=False, tickers_only=False):
  entity_pairs = [e.split(',') for e in list(set([x.strip() for x in open(filename)][1:]))]
