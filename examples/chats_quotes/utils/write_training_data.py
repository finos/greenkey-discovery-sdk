...
def concat_data(outfile, list_of_files):
  ...
  data = list(chain.from_iterable([Path(infile).read_text().splitlines() for infile in list_of_files if
                                   exists(infile) and is_file(infile) and infile.endswith('.txt')]))
...
def write_intents_config():
  ...
  train_quotes = "train_quotes_{}.txt".format(int(TRAIN_SIZE * 100))
  train_not_quotes = "train_not_quotes_{}.txt".format(int(TRAIN_SIZE * 100))
  ...
  try:
    ...
    print(
      "The `custom/` directory must {} and all training files named exactly as specified. If any files are missing or data file paths are invalid, Discovery will NOT train an intents classifier upon launch".format(
...
def main():
  ...
  train_files, test_files = train_test_split_and_write(directory=data_dir, train_dir=custom_dir, test_dir=DIRECTORY,
                                                       size=TRAIN_SIZE, ext='.txt')
...
if __name__ == '__main__':
  ...
  try:
    TRAIN_SIZE = float(sys.argv[2])    # TODO change to argparse
  except IndexError:
    TRAIN_SIZE = 0.8
