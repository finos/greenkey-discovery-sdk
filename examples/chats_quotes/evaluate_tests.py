...
def test_dict(result, verbose=False):
	...
	test_dict = dict(
		test_no=test_no, test_transcript=test_transcript,
		transcript=clean_up(test_transcript), expected_intent=expected_intent
	)
	if verbose:
		pp.pprint(test_dict)
	return test_dict
...
def pred_intent(segment, confidence_threshold=0.5):
	prob, intent_label = sorted([
		(intent_dict["probability"], intent_dict["label"]) for intent_dict in segment["intents"]
	], reverse=True)[0]
