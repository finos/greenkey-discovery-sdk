...
def concat_data(outfile, list_of_files):
  ...
  """
    ...
    data = list(
        chain.from_iterable(
            [
                Path(infile).read_text().splitlines()
                for infile in list_of_files
                if exists(infile) and is_file(infile) and infile.endswith('.txt')
            ]
        )
    )
...
def write_tests(test_files=None):
  ...
  """
    ...
    try:
        subprocess.call(
            "python write_test_data.py {} {} {}".format(quotes_test, not_quotes_test, test_outfile), shell=True
        )
...
def write_intents_config():
  ...
  """
    ...
    intents_config = {
        "intents":
            [
                dict(label='quote', domain='quote_or_not', examples=["/custom/{}".format(train_quotes)]),
                dict(label='not_quote', domain='quote_or_not', examples=["/custom/{}".format(train_not_quotes)])
            ]
    }
    ...
    try:
        assert all(
            [exists(join_path(custom_dir, intent_file)) in os.listdir(custom_dir) for intent_file in training_data]
        )
    except AssertionError:
        print(
            "The `custom/` directory must {} and all training files named exactly as specified. If any files are missing or data file paths are invalid, Discovery will NOT train an intents classifier upon launch"
            .format(DEFINITIONS_FILENAME)
        )
...
def main():
  ...
  """
    ...
    data_grouped = [  # (outfile, args),
        ('quotes.txt', quote_files), ('not_quotes.txt', not_quote_files)
    ]
    ...
    train_files, test_files = train_test_split_and_write(
        directory=data_dir, train_dir=custom_dir, test_dir=DIRECTORY, size=TRAIN_SIZE, ext='.txt'
    )
