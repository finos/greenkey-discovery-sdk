...
with open(inpath, 'r+', encoding='utf-8') as f:
    ...
    out = [
        tuple(line.strip().lower().split(":"))
        for line in list_of_lines
        if line.strip() and line.split(":")[0].split()[0].lower().strip() in ['testing', 'expected', 'observed']
    ]
    # print(out[:10])
...
tests = list(map(dict, bucket(items=[(key.strip().split()[0], val.strip().split()[-1]) for key, val in out], n=3)))
...
labels = [['pred_quotes', 'pred_not_quotes'], ['true_quotes', 'true_not_quotes']]
# labels = ['quotes', 'not_quotes']
cm = my_confusion_matrix = [[TP, FN], [FP, TN]]

q_count = TP + FN
not_q_count = FP + TN
normalized_confusion_matrix = [
    [round(TP / q_count, 3), round(FN / q_count, 3)], [round(FP / not_q_count, 3),
                                                       round(TN / not_q_count, 3)]
]
